...
def check_openings(driver: any, course: str, section: str, crn: str, remaining_seats: int) -> int:
    """ updates seat information for given class, sends email if the class opens up or the class closes, returns the
    updated amount of seats left"""
    ...
    driver.execute_script("await new Promise(r => setTimeout(r, 2000));")
    driver.execute_script("console.log(document.getElementsByClassName(['course-section-all-sections-seats'])[" + str(
        int(section) - 1) + "].textContent);")
    data_text = driver.get_log('browser')[0]['message']
    updated_seats_left = int(data_text[26:len(data_text) - 1])
    if (remaining_seats <= 0 < updated_seats_left) or (remaining_seats > 0 >= updated_seats_left):
    ...
    if os.path.exists('classes.txt'):
        ...
        with open('classes.txt', 'r') as fh:
            ...
            for line in lines:
                ...
                openings.append(
                    f'{data[0]}: {data[1]}, Section: {data[3]}, CRN: {data[5]}, Seats: {updated_seats_left}\n')
        if not openings:
